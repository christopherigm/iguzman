...
const RELEASE_NAME = 'redis';
const CHART_PATH = './packages/charts/redis';
...
const authPassword = await prompt('  auth.password', '');
...
const setFlags = [
  `--set auth.password=${authPassword}`,
...
console.log(`  auth.password:      ${'*'.repeat(authPassword.length)}`);
...
console.log(`\n  Deploying Redis to namespace "${namespace}"...\n`);

try {
  execSync(command, { cwd: ROOT, stdio: 'inherit' });
  console.log(
    `\n  ✓ Redis deployed successfully to namespace "${namespace}".\n`,
  );
} catch {
