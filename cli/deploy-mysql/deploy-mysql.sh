...
# deploy-mysql.sh
#
# Interactive Helm deployment for the custom MySQL chart.
...
#
# Run: bash cli/deploy-mysql/deploy-mysql.sh
...
setup_strings() {
  ...
  if [[ "${lang}" == "es" ]]; then
    ...
    AUTH_MODE_PROMPT="Modo de autenticación (MYSQL_ROOT_PASSWORD)"
    ...
    SECRET_KEY_SELECT="Selecciona la clave de contraseña root"
    ...
    ROOT_PASSWORD_PROMPT="Contraseña root de MySQL"
    ROOT_PASSWORD_REQUIRED="La contraseña root es requerida."
    DATABASE_PROMPT="auth.database"
    DATABASE_REQUIRED="La base de datos es requerida."
    USER_PROMPT="auth.user"
    USER_REQUIRED="El usuario es requerido."
    PASSWORD_PROMPT="auth.password (contraseña del usuario)"
    PASSWORD_REQUIRED="La contraseña del usuario es requerida."
    ...
    SUMMARY_CHART="Chart"
    SUMMARY_AUTH="Auth (root)"
    SUMMARY_SECRET="Secreto"
    SUMMARY_KEY="Clave"
    SUMMARY_ROOT_PASSWORD="Contraseña root"
    SUMMARY_DATABASE="auth.database"
    SUMMARY_USER="auth.user"
    SUMMARY_PASSWORD="auth.password"
    SUMMARY_SIZE="persistence.size"
    SUMMARY_COMMAND="Comando"
    CONFIRM_PROMPT="¿Continuar? [s/n]"
    CONFIRM_YES_CHARS="sy"
    CANCELLED="Cancelado."
    DEPLOYING="Desplegando MySQL en el namespace"
    ...
    AUTH_MODE_PROMPT="Authentication mode (MYSQL_ROOT_PASSWORD)"
    ...
    SECRET_KEY_SELECT="Select root password key"
    ...
    ROOT_PASSWORD_PROMPT="MySQL root password"
    ROOT_PASSWORD_REQUIRED="Root password is required."
    DATABASE_PROMPT="auth.database"
    DATABASE_REQUIRED="Database is required."
    USER_PROMPT="auth.user"
    USER_REQUIRED="User is required."
    PASSWORD_PROMPT="auth.password (user password)"
    ...
    SUMMARY_KEY="Key"
    SUMMARY_ROOT_PASSWORD="Root password"
    SUMMARY_DATABASE="auth.database"
    SUMMARY_USER="auth.user"
    SUMMARY_PASSWORD="auth.password"
    SUMMARY_SIZE="persistence.size"
    ...
    CONFIRM_YES_CHARS="y"
    CANCELLED="Cancelled."
    DEPLOYING="Deploying MySQL to namespace"
    SUCCEEDED="MySQL deployed successfully to namespace"
...
# ── MySQL root password key discovery ────────────────────────────────────────
# Reads an env.example and fills PASSWORD_KEYS[] with keys matching common
# MySQL root password patterns: MYSQL_ROOT_PASSWORD, DB_ROOT_PASSWORD, etc.
...
discover_password_keys() {
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    if [[ "${line}" =~ ^[[:space:]]*([A-Za-z_][A-Za-z0-9_]*)= ]]; then
      ...
      if [[ "${k}" =~ ^(MYSQL_(ROOT_)?PASSWORD|DB_ROOT_PASSWORD)$ ]]; then
        PASSWORD_KEYS+=("${k}")
      fi
    fi
  done < "${file}"
...
main() {
  ...
  local release_name="mysql"
  local chart_path="${repo_root}/packages/charts/mysql"
  ...
  local -a set_args=()
  local summary_auth="" summary_secret="" summary_key="" summary_root_password=""
  ...
  if [[ "${auth_mode}" -eq 0 ]]; then
    ...
    local root_password
    root_password="$(prompt_secret "${ROOT_PASSWORD_PROMPT}")"
    if [[ -z "${root_password}" ]]; then
      printf "  %s\n\n" "$(clr_bold_red "${ROOT_PASSWORD_REQUIRED}")"
      exit 1
    fi
    set_args+=(--set "auth.rootPassword=${root_password}")
    summary_auth="${AUTH_MODE_DIRECT}"
    summary_root_password="${root_password//?/*}"
    display_cmd_auth="--set auth.rootPassword=${summary_root_password}"
  fi

  # ── Database, user, user password ────────────────────────────────────────────

  local db_name db_user db_password
  db_name="$(prompt_visible "${DATABASE_PROMPT}")"
  echo ""
  if [[ -z "${db_name}" ]]; then
    printf "  %s\n\n" "$(clr_bold_red "${DATABASE_REQUIRED}")"
    exit 1
  fi

  db_user="$(prompt_visible "${USER_PROMPT}")"
  echo ""
  if [[ -z "${db_user}" ]]; then
    printf "  %s\n\n" "$(clr_bold_red "${USER_REQUIRED}")"
    exit 1
  fi

  db_password="$(prompt_secret "${PASSWORD_PROMPT}")"
  ...
  local masked_db_password="${db_password//?/*}"
  local display_cmd="helm upgrade --install ${release_name} ./packages/charts/mysql --namespace ${namespace} --create-namespace ${display_cmd_auth} --set auth.database=${db_name} --set auth.user=${db_user} --set auth.password=${masked_db_password} --set persistence.size=${pvc_size}"
  ...
  printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_CHART}:")"     "./packages/charts/mysql"
  printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_AUTH}:")"      "${summary_auth}"
  if [[ "${auth_mode}" -eq 0 ]]; then
    printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_SECRET}:")"       "${summary_secret}"
    printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_KEY}:")"          "${summary_key}"
  else
    printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_ROOT_PASSWORD}:")" "${summary_root_password}"
  fi
  printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_DATABASE}:")"  "${db_name}"
  printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_USER}:")"      "${db_user}"
  printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_PASSWORD}:")"  "${masked_db_password}"
