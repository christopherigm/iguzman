...
setup_strings() {
  local lang="$1"
  if [[ "${lang}" == "es" ]]; then
    WELCOME="MongoDB – Despliegue con Helm"
    SUBTITLE="Instala o actualiza MongoDB en tu clúster de Kubernetes."
    ...
    PASSWORD_PROMPT="Contraseña root de MongoDB (opcional)"
    ...
    SUMMARY_ROOT_USER="auth.rootUsername"
    SUMMARY_SIZE="persistence.size"
    SUMMARY_COMMAND="Comando"
    CONFIRM_PROMPT="¿Continuar? [s/n]"
    CONFIRM_YES_CHARS="sy"
    CANCELLED="Cancelado."
    DEPLOYING="Desplegando MongoDB en el namespace"
    SUCCEEDED="MongoDB desplegado exitosamente en el namespace"
    FAILED="El despliegue con Helm falló."
  else
    WELCOME="MongoDB – Helm Deployment"
    SUBTITLE="Installs or upgrades MongoDB in your Kubernetes cluster."
    ...
    PASSWORD_PROMPT="MongoDB root password (optional)"
    SIZE_PROMPT="persistence.size"
    ...
    CONFIRM_YES_CHARS="y"
    CANCELLED="Cancelled."
    DEPLOYING="Deploying MongoDB to namespace"
    SUCCEEDED="MongoDB deployed successfully to namespace"
...
# ── MongoDB password key discovery ────────────────────────────────────────────
# Reads an env.example and fills PASSWORD_KEYS[] with keys matching common
# MongoDB root password patterns: MONGO_ROOT_PASSWORD, MONGODB_ROOT_PASSWORD, etc.
...
discover_password_keys() {
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    if [[ "${line}" =~ ^[[:space:]]*([A-Za-z_][A-Za-z0-9_]*)= ]]; then
      ...
      if [[ "${k}" =~ ^MONGO(DB)?_(ROOT_)?PASSWORD$ ]]; then
        PASSWORD_KEYS+=("${k}")
      fi
    fi
  done < "${file}"
...
main() {
  ...
  local release_name="mongodb"
  local chart_path="${repo_root}/packages/charts/mongodb"
  ...
  local root_username
  root_username="$(prompt_visible "${ROOT_USER_PROMPT}" "root")"
  echo ""
  ...
  local -a set_args=()
  local summary_auth="" summary_secret="" summary_key="" summary_password="(none)"
  ...
  if [[ "${auth_mode}" -eq 0 ]]; then
    ...
    if [[ -n "${password}" ]]; then
      set_args+=(--set "auth.rootPassword=${password}")
      summary_password="${password//?/*}"
      display_cmd_auth="--set auth.rootPassword=${summary_password}"
    fi
    summary_auth="${AUTH_MODE_DIRECT}"
  fi

  [[ -n "${root_username}" ]] && set_args+=(--set "auth.rootUsername=${root_username}")
  ...
  local display_cmd="helm upgrade --install ${release_name} ./packages/charts/mongodb --namespace ${namespace} --create-namespace --set auth.rootUsername=${root_username} ${display_cmd_auth} --set persistence.size=${pvc_size}"
  ...
  printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_CHART}:")"     "./packages/charts/mongodb"
  printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_ROOT_USER}:")" "${root_username}"
