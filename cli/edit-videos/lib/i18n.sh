...
setup_strings() {
  ...
  if [[ "${lang}" == "es" ]]; then
    ...
    STEP_PREFLIGHT="Comprobando el archivo"
    PREFLIGHT_NO_STREAM="No se encontró un flujo de video legible - se omite el archivo"
    PREFLIGHT_DECODE_FAIL="El archivo no se decodifica (posible corrupción) - se omite el archivo"
    CROPDETECT_TIMEOUT_MSG="La detección de franjas negras excedió el tiempo límite - se omite el archivo"
    ...
    SELECT_FILES_TITLE="Selecciona los archivos a procesar"
    FILE_ALREADY_PROCESSED="ya procesado"
    NO_FILES_SELECTED="Ningún archivo seleccionado. Saliendo."
    ...
    ACTION_COMPRESS="Comprimir reduciendo la calidad (porcentaje)"
    COMPRESS_PERCENT_LABEL="Nivel de compresión % (más alto = más pequeño / menor calidad) [1-100]"
    COMPRESS_RESULT_LABEL="Comprimido"
    COMPRESS_SMALLER_LABEL="más pequeño"
    COMPRESS_LARGER_LABEL="más grande"
    ...
    CONTAINER_SELECT_LABEL="Contenedor de salida"
    CONTAINER_SOURCE_OPTION="Igual que el original - conserva la extensión de cada archivo"
    CONTAINER_MP4_OPTION="MP4 - máxima compatibilidad; los subtítulos de imagen (PGS/VobSub) se descartan y ASS pierde formato"
    CONTAINER_MKV_OPTION="MKV - conserva todos los subtítulos sin pérdida (SRT, ASS, PGS, VobSub)"
    ACTION_MPG_TO_MP4="Convertir MPG/MPEG a MP4 / recodificar MKV (H.264/H.265)"
    MPG_TO_MP4_PRECONVERT="Convirtiendo MPG/MPEG a MP4"
    MPG_TO_MP4_SKIP_MSG="No es MPG/MPEG, se omite la conversión"
    ACTION_SMARTTV="Perfil Smart TV (Tizen) - MP4 H.265, recorta bandas, audio apto para TV"
    SMARTTV_TIER_LABEL="Calidad Smart TV [1=SD 480p, 2=HD 720p, 3=FHD 1080p]"
    SMARTTV_INFO="Recorta bandas negras, escala a la resolución elegida, recodifica a H.265 y convierte el audio DTS/TrueHD a AC3 en un MP4 reproducible en la TV (faststart + hvc1)"
    OCR_IMAGE_SUBS_FOUND="archivo(s) con subtítulos de imagen (DVD/VobSub)"
    OCR_INFO="MP4 no puede almacenar subtítulos de imagen. El OCR los convierte a texto (mov_text) sin quemarlos en el video."
    OCR_LANGS_LABEL="Idiomas detectados"
    OCR_PROMPT="¿Convertir los subtítulos de imagen a texto con OCR?"
    OCR_CHECK="Verificando tesseract..."
    OCR_FOUND="OCR listo"
    OCR_NOT_FOUND="tesseract no encontrado"
    OCR_INSTALL_PROMPT="¿Instalar tesseract automáticamente? (requiere sudo)"
    OCR_INSTALL_RUNNING="Instalando dependencias de OCR vía apt (requiere sudo)..."
    OCR_INSTALL_FAIL="La instalación del OCR falló"
    OCR_MANUAL_HINT="Instálalo manualmente y vuelve a ejecutar:"
    OCR_NO_APT="apt-get no está disponible en este sistema"
    OCR_DISABLED_WARN="OCR desactivado - los subtítulos de imagen se descartarán."
    OCR_SUBS_DROPPED_WARN="Los subtítulos de imagen se descartarán (MP4 no puede contenerlos)."
    OCR_SLOW_WARN="El OCR añade varios minutos por pista y puede cometer errores de reconocimiento."
    OCR_LANG_MISSING="Faltan datos de idioma de tesseract"
    OCR_STEP="OCR de subtítulos"
    OCR_STREAM_FAIL="No se reconoció texto - pista omitida"
    OCR_SUMMARY_LABEL="OCR de subtítulos"
    SKIP_EXISTING_FOUND="archivos ya encontrados en la carpeta de salida"
    SKIP_EXISTING_PROMPT="¿Omitir archivos ya procesados?"
    FILES_NOT_SELECTED="sin seleccionar"
    ACTION_TIKTOK="Crear Reel TikTok/Reels - IA (Ollama + Gemma, carpeta → 1 reel vertical)"
    TIKTOK_TITLE="Ensamblando Reel TikTok"
    TIKTOK_PHASE1="Fase 1/3 - Puntuación de fotogramas con LLM"
    TIKTOK_PHASE2="Fase 2/3 - Recorte vertical + normalización por clip"
    TIKTOK_PHASE3="Fase 3/3 - Concatenación + mezcla de audio"
    TIKTOK_STEP_ROI="Detectando región de interés"
    TIKTOK_STEP_NORMALIZE="Normalizando a 1080x1920 @ 30fps"
    TIKTOK_STEP_CONCAT="Concatenando clips"
    TIKTOK_STEP_AUDIO="Mezclando pistas de audio"
    TIKTOK_BEST_SCORE="Mejor puntuación de fotograma"
    TIKTOK_SELECTED="Seleccionado (puntuación ≥ umbral)"
    TIKTOK_SKIPPED="Omitido (puntuación baja)"
    TIKTOK_NO_CLIPS_SELECTED="Ningún clip superó el umbral de calidad"
    TIKTOK_CLIPS_SELECTED="clips seleccionados"
    TIKTOK_CLIP_FAIL="Codificación del clip fallida - se omite"
    TIKTOK_CLIP_DONE="Clip listo"
    TIKTOK_NO_CLIPS_ENCODED="Ningún clip fue codificado correctamente"
    TIKTOK_CONCAT_FAIL="La concatenación falló"
    TIKTOK_DONE="¡Reel listo!"
    TIKTOK_DEDUP_SKIP="Omitido por similitud con otra foto"
    TIKTOK_DEDUP_REPLACED="Reemplazado por foto más nítida"
    TIKTOK_OLLAMA_CHECK="Verificando Ollama..."
    TIKTOK_OLLAMA_FOUND="Ollama encontrado"
    TIKTOK_OLLAMA_NOT_FOUND="Ollama no encontrado o no está en ejecución"
    TIKTOK_OLLAMA_MODEL_NOT_FOUND="Modelo no encontrado en Ollama"
    TIKTOK_OLLAMA_PULL_PROMPT="¿Descargar el modelo automáticamente?"
    TIKTOK_OLLAMA_PULLING="Descargando modelo desde Ollama..."
    TIKTOK_OLLAMA_PULL_FAIL="No se pudo descargar el modelo. Ejecuta: ollama pull"
    TIKTOK_MIN_SCORE_PROMPT="Puntuación mínima de calidad para incluir clips [1-10]"
    TIKTOK_CLIP_MIN_PROMPT="Duración mínima de clip en segundos (clips con puntuación baja)"
    TIKTOK_CLIP_MAX_PROMPT="Duración máxima de clip en segundos (clips con puntuación alta)"
    TIKTOK_FRAME_INTERVAL_PROMPT="Intervalo de extracción de fotogramas en segundos [1-30]"
    TIKTOK_MODEL_PROMPT="Nombre del modelo Ollama"
    TIKTOK_MUSIC_PROMPT="Ruta del archivo de música de fondo (vacío = sin música)"
    TIKTOK_MUSIC_VOL_PROMPT="Volumen de la música de fondo [0.0-1.0]"
    TIKTOK_ORIG_VOL_PROMPT="Volumen del audio original [0.0-1.0]"
    TIKTOK_DISK_WARN="Aviso: el modo TikTok extrae fotogramas e intermedios a /tmp. Requiere espacio en disco."
    TIKTOK_NOTE="Nota: el modo TikTok omite el procesamiento por archivo y ensambla un único reel vertical."
    STREAM_INSPECTING="Inspeccionando los videos en busca de pistas de audio y subtítulos..."
    STREAM_SELECT_TITLE="Manejo de audio y subtítulos"
    STREAM_KEEP_ALL="Conservar todas las pistas de audio y subtítulos"
    STREAM_SELECT_PER_FILE="Seleccionar pistas y subtítulos por video antes de procesar"
    STREAM_SELECT_FILE_PROMPT="Selecciona las pistas de audio y subtítulos a conservar"
    STREAM_VIDEO_KEPT_NOTE="(la pista de video siempre se conserva)"
    STREAM_AUDIO_LABEL="Audio"
    STREAM_SUBTITLE_LABEL="Subtítulo"
    STREAM_NO_LANG="und"
    STREAM_NONE_SELECTED_WARN="No se seleccionó audio ni subtítulos para este archivo - solo se conservará el video."
  else
    ...
    STEP_PREFLIGHT="Checking file"
    PREFLIGHT_NO_STREAM="No readable video stream found - skipping file"
    PREFLIGHT_DECODE_FAIL="File does not decode (possible corruption) - skipping file"
    CROPDETECT_TIMEOUT_MSG="Black-bar detection timed out - skipping file"
    ...
    SELECT_FILES_TITLE="Select files to process"
    FILE_ALREADY_PROCESSED="already processed"
    NO_FILES_SELECTED="No files selected. Exiting."
    ...
    ACTION_COMPRESS="Compress by reducing quality (percentage)"
    COMPRESS_PERCENT_LABEL="Compression level % (higher = smaller / lower quality) [1-100]"
    COMPRESS_RESULT_LABEL="Compressed"
    COMPRESS_SMALLER_LABEL="smaller"
    COMPRESS_LARGER_LABEL="larger"
    ...
    CONTAINER_SELECT_LABEL="Output container"
    CONTAINER_SOURCE_OPTION="Same as input - keep each file's original extension"
    CONTAINER_MP4_OPTION="MP4 - maximum compatibility; image subtitles (PGS/VobSub) are dropped and ASS loses styling"
    CONTAINER_MKV_OPTION="MKV - preserves every subtitle losslessly (SRT, ASS, PGS, VobSub)"
    ACTION_MPG_TO_MP4="Convert MPG/MPEG to MP4 / re-encode MKV (H.264/H.265)"
    MPG_TO_MP4_PRECONVERT="Converting MPG/MPEG to MP4"
    MPG_TO_MP4_SKIP_MSG="Not MPG/MPEG, skipping conversion"
    ACTION_SMARTTV="Smart TV (Tizen) profile - MP4 H.265, crop bars, TV-friendly audio"
    SMARTTV_TIER_LABEL="Smart TV quality [1=SD 480p, 2=HD 720p, 3=FHD 1080p]"
    SMARTTV_INFO="Crops black bars, scales to the chosen resolution, re-encodes to H.265, and converts DTS/TrueHD audio to AC3 in a TV-playable MP4 (faststart + hvc1)"
    OCR_IMAGE_SUBS_FOUND="file(s) with image subtitles (DVD/VobSub)"
    OCR_INFO="MP4 cannot store image subtitles. OCR converts them to text (mov_text) without burning them into the video."
    OCR_LANGS_LABEL="Detected languages"
    OCR_PROMPT="OCR image subtitles to text?"
    OCR_CHECK="Checking tesseract..."
    OCR_FOUND="OCR ready"
    OCR_NOT_FOUND="tesseract not found"
    OCR_INSTALL_PROMPT="Install tesseract automatically? (requires sudo)"
    OCR_INSTALL_RUNNING="Installing OCR dependencies via apt (requires sudo)..."
    OCR_INSTALL_FAIL="OCR installation failed"
    OCR_MANUAL_HINT="Install it manually, then re-run:"
    OCR_NO_APT="apt-get is not available on this system"
    OCR_DISABLED_WARN="OCR disabled - image subtitles will be dropped."
    OCR_SUBS_DROPPED_WARN="Image subtitles will be dropped (MP4 cannot hold them)."
    OCR_SLOW_WARN="OCR adds several minutes per track and can make recognition errors."
    OCR_LANG_MISSING="Missing tesseract language data"
    OCR_STEP="OCR subtitles"
    OCR_STREAM_FAIL="No text recognised - track skipped"
    OCR_SUMMARY_LABEL="Subtitle OCR"
    SKIP_EXISTING_FOUND="files already found in output folder"
    SKIP_EXISTING_PROMPT="Skip already processed files?"
    FILES_NOT_SELECTED="not selected"
    ACTION_TIKTOK="Create TikTok/Reel - AI (Ollama + Gemma, folder → 1 vertical reel)"
    TIKTOK_TITLE="Assembling TikTok Reel"
    TIKTOK_PHASE1="Phase 1/3 - LLM frame scoring"
    TIKTOK_PHASE2="Phase 2/3 - Vertical smart-crop + per-clip normalization"
    TIKTOK_PHASE3="Phase 3/3 - Concat + audio mix"
    TIKTOK_STEP_ROI="Detecting region of interest"
    TIKTOK_STEP_NORMALIZE="Normalizing to 1080x1920 @ 30fps"
    TIKTOK_STEP_CONCAT="Concatenating clips"
    TIKTOK_STEP_AUDIO="Mixing audio tracks"
    TIKTOK_BEST_SCORE="Best frame score"
    TIKTOK_SELECTED="Selected (score ≥ threshold)"
    TIKTOK_SKIPPED="Skipped (low score)"
    TIKTOK_NO_CLIPS_SELECTED="No clips passed the quality threshold"
    TIKTOK_CLIPS_SELECTED="clips selected"
    TIKTOK_CLIP_FAIL="Clip encoding failed - skipping"
    TIKTOK_CLIP_DONE="Clip ready"
    TIKTOK_NO_CLIPS_ENCODED="No clips were successfully encoded"
    TIKTOK_CONCAT_FAIL="Concatenation failed"
    TIKTOK_DONE="Reel ready!"
    TIKTOK_DEDUP_SKIP="Skipped - too similar to another photo"
    TIKTOK_DEDUP_REPLACED="Replaced by sharper photo"
    TIKTOK_OLLAMA_CHECK="Checking Ollama..."
    TIKTOK_OLLAMA_FOUND="Ollama found"
    TIKTOK_OLLAMA_NOT_FOUND="Ollama not found or not running"
    TIKTOK_OLLAMA_MODEL_NOT_FOUND="Model not found in Ollama"
    TIKTOK_OLLAMA_PULL_PROMPT="Pull model automatically?"
    TIKTOK_OLLAMA_PULLING="Pulling model from Ollama..."
    TIKTOK_OLLAMA_PULL_FAIL="Could not pull model. Run: ollama pull"
    TIKTOK_MIN_SCORE_PROMPT="Minimum quality score to include clips [1-10]"
    TIKTOK_CLIP_MIN_PROMPT="Minimum clip duration in seconds (low-score clips)"
    TIKTOK_CLIP_MAX_PROMPT="Maximum clip duration in seconds (high-score clips)"
    TIKTOK_FRAME_INTERVAL_PROMPT="Frame extraction interval in seconds [1-30]"
    TIKTOK_MODEL_PROMPT="Ollama model name"
    TIKTOK_MUSIC_PROMPT="Background music file path (leave empty for no music)"
    TIKTOK_MUSIC_VOL_PROMPT="Background music volume [0.0-1.0]"
    TIKTOK_ORIG_VOL_PROMPT="Original audio volume [0.0-1.0]"
    TIKTOK_DISK_WARN="Warning: TikTok mode extracts frames and intermediates to /tmp. Ensure sufficient disk space."
    TIKTOK_NOTE="Note: TikTok mode bypasses per-file processing and assembles a single vertical reel."
    STREAM_INSPECTING="Inspecting videos for audio & subtitle tracks..."
    STREAM_SELECT_TITLE="Audio & subtitle handling"
    STREAM_KEEP_ALL="Keep all audio streams & subtitles/captions"
    STREAM_SELECT_PER_FILE="Select streams & subtitles per video before processing"
    STREAM_SELECT_FILE_PROMPT="Select audio & subtitle tracks to keep"
    STREAM_VIDEO_KEPT_NOTE="(the video stream is always kept)"
    STREAM_AUDIO_LABEL="Audio"
    STREAM_SUBTITLE_LABEL="Subtitle"
    STREAM_NO_LANG="und"
    STREAM_NONE_SELECTED_WARN="No audio or subtitle tracks selected for this file - only video will be kept."
  fi
}
