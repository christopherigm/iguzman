...
# deploy-postgres.sh
#
# Interactive Helm deployment for the custom PostgreSQL chart.
...
#
# Run: bash cli/deploy-postgres/deploy-postgres.sh
...
setup_strings() {
  local lang="$1"
  if [[ "${lang}" == "es" ]]; then
    WELCOME="PostgreSQL – Despliegue con Helm"
    SUBTITLE="Instala o actualiza PostgreSQL en tu clúster de Kubernetes."
    ...
    AUTH_MODE_PROMPT="Modo de autenticación (POSTGRES_PASSWORD)"
    ...
    PASSWORD_PROMPT="Contraseña de PostgreSQL"
    ...
    CANCELLED="Cancelado."
    DEPLOYING="Desplegando PostgreSQL en el namespace"
    SUCCEEDED="PostgreSQL desplegado exitosamente en el namespace"
    FAILED="El despliegue con Helm falló."
  else
    WELCOME="PostgreSQL – Helm Deployment"
    SUBTITLE="Installs or upgrades PostgreSQL in your Kubernetes cluster."
    ...
    AUTH_MODE_PROMPT="Authentication mode (POSTGRES_PASSWORD)"
    ...
    PASSWORD_PROMPT="PostgreSQL password"
    ...
    CONFIRM_YES_CHARS="y"
    CANCELLED="Cancelled."
    DEPLOYING="Deploying PostgreSQL to namespace"
    SUCCEEDED="PostgreSQL deployed successfully to namespace"
...
# ── DB password key discovery ─────────────────────────────────────────────────
# Reads an env.example and fills DB_KEYS[] with keys whose names match common
# database password patterns: DB_PASSWORD, DATABASE_PASSWORD, POSTGRES_PASSWORD, etc.
...
discover_db_keys() {
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    if [[ "${line}" =~ ^[[:space:]]*([A-Za-z_][A-Za-z0-9_]*)= ]]; then
      ...
      if [[ "${k}" =~ ^(DB|DATABASE|POSTGRES(QL)?)_PASSWORD$ ]]; then
        DB_KEYS+=("${k}")
      fi
    fi
  done < "${file}"
...
main() {
  ...
  local release_name="postgres"
  local chart_path="${repo_root}/packages/charts/postgres"
  ...
  if [[ "${auth_mode}" -eq 0 ]]; then
    ...
    local -a DB_KEYS=()
    discover_db_keys "${app_env}"

    local secret_key default_key=""
    if [[ ${#DB_KEYS[@]} -gt 1 ]]; then
      printf "  %s:\n\n" "$(clr_bold "${SECRET_KEY_SELECT}")"
      MENU_ITEMS=("${DB_KEYS[@]}")
      MENU_SELECTED=0
      interactive_select
      default_key="${DB_KEYS[${MENU_SELECTED}]}"
      echo ""
    elif [[ ${#DB_KEYS[@]} -eq 1 ]]; then
      default_key="${DB_KEYS[0]}"
    fi
  ...
  pg_user="$(prompt_visible "${USER_PROMPT}" "postgres")"
  echo ""
  pg_database="$(prompt_visible "${DATABASE_PROMPT}" "postgres")"
  echo ""
  set_args+=(--set "auth.user=${pg_user}" --set "auth.database=${pg_database}")
  ...
  local display_cmd="helm upgrade --install ${release_name} ./packages/charts/postgres --namespace ${namespace} --create-namespace ${display_cmd_auth} --set auth.user=${pg_user} --set auth.database=${pg_database} --set persistence.size=${pvc_size}"
  ...
  printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_CHART}:")"     "./packages/charts/postgres"
  ...
  printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_USER}:")"      "${pg_user}"
  printf "  %-22s %s\n" "$(clr_dim "${SUMMARY_DATABASE}:")"  "${pg_database}"
