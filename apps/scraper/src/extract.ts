...
export async function extractUrl(url: string): Promise<ExtractResult> {
  ...
  try {
    ...
    const result = await page.evaluate((): { title: string; content: string; og_image: string | null } => {
    ...
    return { title: result.title, url, content: result.content, og_image: result.og_image };
  } finally {
    await page.close();
  }
}
