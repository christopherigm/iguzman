...
export function ScraperPanel() {
  ...
  const runHealth = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setHealthError({ msg: t('scraperErrorServer', { status: res.status }), id: Date.now() });
      }
    } catch {
  ...
  const runSearch = async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        setSearchError({ msg: t('scraperErrorServer', { status: res.status }), id: Date.now() });
      }
    } catch {
  ...
  const runExtract = async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        setExtractError({ msg: t('scraperErrorServer', { status: res.status }), id: Date.now() });
      }
    } catch {
  ...
  return (
    <>
      <Box flexDirection="column" gap={8} marginBottom={40}>
        <Typography as="h2" variant="h3">{t('scraperSection')}</Typography>
      ...
      {/* GET /health */}
      <EndpointPanel heading={t('scraperHealthSection')} description={t('scraperHealthDescription')}>
      ...
      {/* POST /search */}
      <EndpointPanel heading={t('scraperSearchSection')} description={t('scraperSearchDescription')}>
        <DocLabel>{t('scraperExampleRequest')}</DocLabel>
        ...
        <Button
          ...
          onClick={runSearch}
        />
        {searchResult !== null && <CodeBlock language="json" code={searchResult} marginTop={4} />}
      </EndpointPanel>
      {searchError && <Toast message={searchError.msg} variant="error" key={searchError.id} />}

      {/* POST /extract */}
      <EndpointPanel heading={t('scraperExtractSection')} description={t('scraperExtractDescription')}>
        <DocLabel>{t('scraperExampleRequest')}</DocLabel>
        ...
        <Button
          ...
          onClick={runExtract}
        />
        {extractResult !== null && <CodeBlock language="json" code={extractResult} marginTop={4} />}
      </EndpointPanel>
      {extractError && <Toast message={extractError.msg} variant="error" key={extractError.id} />}
    </>
  );
}
