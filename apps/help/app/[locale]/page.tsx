...
const CLONE_COMMAND = 'git clone https://github.com/christopherigm/iguzman\ncd iguzman';
...
export default async function Home({ params, searchParams }: Props) {
  ...
  const tab: Tab = VALID_TABS.includes(rawTab as Tab) ? (rawTab as Tab) : 'getting-started';
  ...
  return (
    <>
      <NavbarSpacer />
      <Container size="lg" paddingX={24} styles={{ paddingTop: 40, paddingBottom: 80 }}>
        <Typography as="h1" variant="h2" marginBottom={8}>{heading}</Typography>
        <Typography as="p" variant="body" color="var(--foreground-muted)" marginBottom={40}>
          {subheading}
        </Typography>

        <Box display="flex" gap={40} alignItems="flex-start" className="help-layout">
          ...
          <Box flexDirection="column" flexGrow={1}>
            {tab === 'getting-started' && (
              <>
                <PrereqSection
                  ...
                  items={[
                    { label: t('prereqLinux') },
                    { label: t('prereqDocker'), href: 'https://hub.docker.com/signup' },
                    { label: t('prereqGithub'), href: 'https://github.com' },
                    { label: t('prereqGit'), href: 'https://git-scm.com/downloads', note: t('prereqGitNote') },
                    { label: t('prereqClaude'), href: 'https://claude.ai' },
                  ]}
                />
                <Section heading={t('cloneRepoSection')} description={t('cloneRepoDescription')} code={CLONE_COMMAND} />
                <Section heading={t('setupScriptSection')} description={t('setupScriptDescription')} code={SETUP_SCRIPT_COMMAND} />
                <Section heading={t('sshKeySection')} description={t('sshKeyDescription')} code={SSH_KEY_COMMAND} />
                <Section heading={t('verifySection')} description={t('verifyDescription')} code={VERIFY_COMMANDS} />
              </>
            )}
            ...
            {tab === 'commands' && (
              <>
                ...
                <Section heading={t('newAppSection')} description={t('newAppDescription')} code={NEW_APP_COMMAND} />
                <Section heading={t('newApiSection')} description={t('newApiDescription')} code={NEW_API_COMMAND} />
                <Section heading={t('generateIconsSection')} description={t('generateIconsDescription')} code={GENERATE_ICONS_COMMANDS} />
                <Section heading={t('secretsSection')} description={t('secretsDescription')} code={SECRETS_COMMAND} />
                ...
                <Section heading={t('devSection')} description={t('devDescription')} code={DEV_COMMANDS} />
                <Section heading={t('buildSection')} description={t('buildDescription')} code={BUILD_COMMANDS} />
                <Section heading={t('lintSection')} description={t('lintDescription')} code={LINT_COMMANDS} />
              </>
            )}
...
function Section({ heading, description, code }: { heading: string; description: string; code: string }) {
  return (
      ...
      <Box flexDirection="column" gap={6} marginTop={8}>
        {items.map(({ label, href, note }) => (
          <Box key={label} alignItems="center" gap={8}>
            <Typography as="span" variant="body-sm" color="var(--foreground-muted)">•</Typography>
            {href ? (
              <a href={href} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent)', textDecoration: 'underline', fontSize: 14 }}>
                {label}
              </a>
            ) : (
              <Typography as="span" variant="body-sm">{label}</Typography>
            )}
            {note && (
              <Typography as="span" variant="body-sm" color="var(--foreground-muted)">— {note}</Typography>
            )}
          </Box>
        ))}
      </Box>
