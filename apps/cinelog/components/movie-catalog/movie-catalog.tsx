...
import { deleteMovie, getMovies, getCategories, type Category, type Movie, type MovieFormat } from '@/lib/catalog';
...
export function MovieCatalog() {
  ...
  return (
    <Box flexDirection="column" gap={16} paddingY={16}>
      ...
      {toast && (
        <Toast key={toastKey} message={toast.text} variant={toast.variant} position="top-center" />
      )}
      ...
      <Box display="flex" alignItems="center" justifyContent="space-between" flexWrap="wrap" gap={8}>
        ...
        <Box display="flex" gap={4}>
          <Button
            ...
            color={view === 'grid' ? 'var(--accent-foreground, #ffffff)' : undefined}
          >
            {t('gridView')}
          </Button>
          <Button
            ...
            color={view === 'list' ? 'var(--accent-foreground, #ffffff)' : undefined}
          >
            {t('listView')}
          </Button>
        </Box>
      ...
      {status === 'ready' && movies.length === 0 && (
        <Typography variant="body-sm" textAlign="center" styles={{ opacity: 0.6 }}>
        ...
        <Grid container spacing={2}>
          {movies.map((movie) => (
            <Grid key={movie.id} size={{ xs: 6, sm: 4, md: 3, lg: 2 }}>
              <MovieCard movie={movie} view="grid" onDelete={setPendingDelete} />
            </Grid>
          ))}
        </Grid>
      )}
      ...
      {status === 'ready' && movies.length > 0 && view === 'list' && (
        <Box flexDirection="column" gap={8}>
          {movies.map((movie) => (
            <MovieCard key={movie.id} movie={movie} view="list" onDelete={setPendingDelete} />
          ))}
        </Box>
      )}
