...
export function InboxCard({ item, onAccept, onReject }: Props) {
  ...
  const [year, setYear] = useState(item.extracted_year ? String(item.extracted_year) : '');
  ...
  return (
    <Card flexDirection="column" gap={16} padding={16}>
      <Box display="flex" gap={16} flexWrap="wrap">
        ...
        >
          {item.extracted_cover_url ? (
            <Image src={item.extracted_cover_url} alt="" fill sizes="96px" style={{ objectFit: 'cover' }} />
          ) : (
            <Box
              display="flex"
              alignItems="center"
        ...
        <Box flexDirection="column" gap={6} flex="1 1 200px" minWidth={0}>
          <Box display="flex" gap={6} alignItems="center" flexWrap="wrap">
            ...
            <Badge variant="subtle" size="sm">{item.barcode}</Badge>
          </Box>
          {item.error_message && (
      ...
      <Box display="flex" gap={8} flexWrap="wrap">
        <TextInput label={t('titleLabel')} value={title} onChange={setTitle} flex="2 1 200px" disabled={busy} />
        <TextInput label={t('directorLabel')} value={director} onChange={setDirector} flex="1 1 160px" disabled={busy} />
      </Box>
      ...
      <TextInput label={t('genresLabel')} value={genres} onChange={setGenres} disabled={busy} />
      <TextInput label={t('castLabel')} value={cast} onChange={setCast} disabled={busy} />
