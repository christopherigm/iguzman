...
_(Note: Using the midpoint $T/2$ provides a balanced, fixed linear approximation of the inflation burden over the life of the group)._
...
_Where $B_0$ (Initial Treasury) = $N \times D$_
...
> **Logic:** > 1. Query all active `Group` instances. 2. Call `get_current_balance()` for each group. 3. Calculate daily yield: `(balance * (0.11 / 365))`. 4. Append a new `LedgerEntry` with `transaction_type="YIELD"` for that group.
