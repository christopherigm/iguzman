...
async function runDiarizeJob(
  ...
  const refundOnFailure = () =>
    refundCredits(creditsKey, cost).catch((err: unknown) =>
      log.error({ err, taskId }, 'Failed to refund credits after diarize failure'),
    );
    ...
    if (!alreadyStaged) {
      try {
        ...
        await updateTask(taskId, {
          status: 'error',
          progress: 0,
          error: { code: 'DOWNLOAD_FAILED', message: 'Failed to fetch input file' },
        }).catch(() => {});
        return;
      }
    }
  ...
  try {
    ...
    if (!diarizeRes.ok) {
      ...
      log.error({ status: diarizeRes.status, errText }, 'Diarization service error');
      throw new Error(`Diarization service error (${diarizeRes.status})`);
    }

    const { job_id } = (await diarizeRes.json()) as { job_id: string };
    log.info({ taskId, inputFile, job_id }, 'Diarization job queued, polling for result');
