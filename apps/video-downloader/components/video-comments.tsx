...
function AuthorAvatar({ author, thumbnail }: { author?: string; thumbnail?: string }) {
  const initials = author
    ? author.trim().slice(0, 2).toUpperCase()
    : '?';

  if (thumbnail) {
    return (
      // eslint-disable-next-line @next/next/no-img-element
      <img className="vc-avatar" src={thumbnail} alt={author ?? ''} loading="lazy" />
    );
  }

  return (
    <div className="vc-avatar vc-avatar--initials">
      <Typography variant="caption" fontWeight={600} color="var(--accent-foreground, #fff)">
        {initials}
      </Typography>
    </div>
  );
...
function CommentItem({ comment, replies }: { comment: YtDlpComment; replies: YtDlpComment[] }) {
  const [expanded, setExpanded] = useState(false);

  return (
    <div className="vc-comment">
      <AuthorAvatar author={comment.author} thumbnail={comment.author_thumbnail} />
      <div className="vc-comment-body">
        <div className="vc-comment-meta">
          ...
          {comment.timestamp != null && (
            <Typography variant="caption" color="var(--foreground-muted, #999)" className="vc-date">
        ...
        {(comment.like_count ?? 0) > 0 || comment.is_favorited ? (
          <div className="vc-comment-footer">
            ...
            {comment.is_favorited && (
              <Typography variant="caption" color="var(--accent, #06b6d4)">★</Typography>
            )}
          </div>
        ) : null}
        {replies.length > 0 && (
          ...
          >
            <Typography variant="caption" color="var(--accent, #06b6d4)">
              {expanded ? '▲ Hide replies' : `▼ ${replies.length} repl${replies.length === 1 ? 'y' : 'ies'}`}
            </Typography>
          </button>
        )}
          ...
          <div className="vc-replies">
            {replies.map((reply) => (
              <div key={reply.id} className="vc-comment vc-comment--reply">
                <AuthorAvatar author={reply.author} thumbnail={reply.author_thumbnail} />
                <div className="vc-comment-body">
                  <div className="vc-comment-meta">
                    <Typography variant="caption" fontWeight={600} className="vc-author">
...
export function VideoComments({ commentsUrl, opfsCommentsKey }: VideoCommentsProps) {
