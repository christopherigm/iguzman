...
/**
 * POST /api/backups/restore/ - forward a restore request to Django.
 *
 ...
 *
 * The body is buffered rather than streamed, which is the one place this route
 * trades memory for correctness. `apiFetch` retries once on a 401 (an access
 * token that expired mid-upload), and a `ReadableStream` body can only be
 * consumed once - streaming would turn every expired-token restore into an
...
export async function POST(request: NextRequest) {
  ...
  const data: unknown = await res
    .json()
    .catch(() => ({ detail: "The restore failed." }));
