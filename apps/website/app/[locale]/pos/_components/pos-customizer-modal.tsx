 ...
 *
 * The same rules as the storefront customiser - and literally the same rows
 * (`MenuIngredientPicker`) and the same arithmetic (`lib/menu-selection.ts`), so
 * a bread configured at the till and the same bread configured on the site can
 * never quote different numbers. What differs is the shape: the picker's `lg`
 * size for hit targets a finger can find over a counter, one dish at a time, and
 * a quantity stepper for the dish itself, because an associate rings up "three
 * of these, all with extra seeds" as one action.
 *
