...
/**
 * "Restore" in the System CMS: upload an archive produced by the Backup section
 ...
 * - **Surfaces the server's own message on a 400.** The two failures an operator
 *   will actually hit - an archive from another site, and a section the archive
 *   does not contain - are both explained precisely by the API, and replacing
 *   that with a generic "restore failed" would leave them with nothing to act
 *   on.
 */
  ...
  return (
    <Box flexDirection="column" gap={16} paddingTop={32}>
      ...
      <Box flexWrap="wrap" alignItems="center" gap={12}>
        {/* Hidden because a native file input cannot be styled to match the
            rest of the CMS; the Button below is its label. `aria-hidden` is the
            convention this repo uses for a programmatically-triggered input. */}
        <input
          ref={fileInput}
          type="file"
        ...
        <Typography
          variant="body"
          color={file ? undefined : "var(--muted-foreground, #6b7280)"}
        >
