          ...
          <Box className="buyable-card__pricing">
            ...
            {effectiveCompare &&
              parseFloat(effectiveCompare) > parseFloat(effectivePrice) && (
                <Typography as="span" variant="none" className="buyable-card__compare-price">
                  {formatPrice(effectiveCompare, data.currency)}
                </Typography>
              )}
          </Box>

          {modality && <Typography as="span" variant="none" className="buyable-card__meta">{modality}</Typography>}
          {!modality && variantCount > 1 && (
            <Typography as="span" variant="none" className="buyable-card__meta">{variantCount} variants</Typography>
          )}
        </Box>
      </Box>
    </Link>
...
export async function CatalogItems() {
  ...
  const items: BuyableItem[] = [
    ...
    ...services.map((data): BuyableItem => ({ kind: 'service', data })),
