...
import { captureMapImage } from "@/lib/map-capture";
import "./map-picker.css";

/**
 * A click-to-pick coordinate map, drawn from the tenant's own basemap tiles.
 *
 * ⚠ **Why this is not Google.** Google's keyless embed
 * (`maps.google.com/maps?...&output=embed`) is a cross-origin iframe: nothing on
 * this page can ever read a click inside it. A real picker therefore needs
 * either a Maps JavaScript API key - which this deployment does not have - or a
 * tile source that can be drawn into our own DOM. This is the latter, ported
 * from `apps/animals`' CMS picker so the two authoring surfaces agree.
 *
 * ⚠ **It is not `PlaceMap` / `OsmMap` either, and cannot be.** Those *read* -
 * they frame a location and let a visitor look around; this is a form control
 * that writes two fields, so the click, the pin drag, the place search and the
 * camera adjusted during render are its whole content.
 *
 ...
 * gesture scrim all come from `@repo/ui/core-elements/osm-map-chrome`, which
 * `OsmMap` renders too - so the CMS's map and the customer-facing ones are one
 * design, and a change to either reaches both.
 *
 ...
 * would snag the operator on the way past it. Zooming takes `Ctrl`/`⌘` + wheel
 * (the same event a trackpad pinch produces), the `+`/`-` buttons or the
 * keyboard; a bare wheel raises a scrim saying so and lets the form scroll.
 * Don't "fix" this back to plain wheel zoom.
 *
 * **It also takes the branch's map screenshot.** `capture()` on the imperative
 * handle paints the current camera into a JPEG (`lib/map-capture.ts`) that the
 * branch form saves to `Branch.map_image`, which is what a booking confirmation
 * email shows - an email cannot draw a live map, and this is the one moment
 * where one already exists in a browser.
 */

const MIN_ZOOM = 2;
const MAX_ZOOM = 18;
/** Zoom used once a place is known: a street-sized frame, not a continent. */
const PLACE_ZOOM = 16;
/** Zoom used when nothing at all is known and the map opens on the default centre. */
const WIDE_ZOOM = 4;
/**
 * Where the map opens for a tenant with no pin and nothing to borrow one from:
 * the geographic middle of Mexico, which is where this platform's customers are.
 */
const DEFAULT_CENTER = { latitude: 23.6345, longitude: -102.5528 };
/** What the API stores: `DecimalField(max_digits=10, decimal_places=8)`. */
 ...
 * a drag rather than as a new click.
 */
...
/** What the branch form reaches into the picker for when it saves. */
export interface MapPickerHandle {
  /**
   * A JPEG data URL of the current pin on the current camera, or `null` when
   * the browser could not produce one (no pin, a tile host with no CORS header,
   * an offline moment). `null` is an ordinary answer - the coordinates save
   * regardless; see `lib/map-capture.ts`.
   */
...
export interface MapPickerProps {
  ...
  /**
   * The mark drawn inside the captured pin - the tenant's **brandmark**, not
   * its logo, exactly as `PlaceMap` explains. With none, the captured pin is a
   * plain accent teardrop.
   */
  pinIcon?: string | null;
...
export function MapPicker({
  ...
  // The same tiles and the same credit the public maps draw - one tenant
  // setting, resolved in the layout. An operator placing a pin has to be
  // looking at the map a customer will see it on, and the screenshot this
  // captures is drawn from the same basemap for the same reason.
  ...
  const [center, setCenter] = useState<LatLng>(() => pin ?? DEFAULT_CENTER);
  const [zoom, setZoom] = useState<number>(() =>
    pin ? PLACE_ZOOM : WIDE_ZOOM,
  );
  ...
  /** Where the operator is - `null` until the browser says, and if it never does. */
   ...
   * changed *elsewhere*, but it must not follow our own click - or every click
   * would yank the map so the new pin sat dead centre under the cursor.
   */
  ...
  // *derivation* of the pin that the operator then takes over by panning, which
  // is exactly the case React's "adjusting state when a prop changes" pattern
  // covers - and it re-renders before painting, so the map never shows one frame
  // at the old centre.
  //
  // The key is a string, not an object, because the parent re-renders on every
  // keystroke anywhere in the form; keyed on identity this would re-centre the
  // map - undoing the operator's panning - each time they typed.
  const pinKey = `${latitude},${longitude}`;
  const [syncedPin, setSyncedPin] = useState(pinKey);
  if (syncedPin !== pinKey) {
    const followPin = pinKey !== emitted && pin !== null;
    setSyncedPin(pinKey);
    if (followPin) {
      setCenter(pin);
  ...
  // ── The screenshot ────────────────────────────────────────────────────────
  //
  // Imperative rather than a `useEffect` that fires on every pin change: the
  // capture loads six tiles over the network and encodes a JPEG, which is not
  // something to do on each nudge of a pin - the branch form asks for one at the
  // moment it saves, and only when the pin actually moved.
  const basemapRef = useRef(basemap);
  const pinIconRef = useRef(pinIcon);
  useEffect(() => {
    ...
    () => ({
      capture: async () => {
        ...
        return captureMapImage({
          center: current,
          // The camera the operator left the map on, so a rural location can be
          // captured wide and a shop on a street captured close.
          zoom: cameraRef.current.zoom,
          tileUrl: basemapRef.current.tileUrl,
          attribution: basemapRef.current.attribution,
  ...
  //
  // Drawn as a second, hollow pin so a branch can be pinned at the spot the
  // operator is standing in - the common case for someone setting up the CMS
  // from the shop. It never writes the form: the coordinate this control
  // produces is the one that gets clicked, so a fix arriving mid-edit cannot
  // move a pin already placed. Every failure - refused, no fix, an insecure
  // origin - leaves it null and the picker exactly as it was.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handlePointerUp = (e: PointerEvent) => {
      ...
      if (finished.onPin) {
        if (finished.moved) {
          ...
          emitRef.current(
            fromWorld(
              { x: world.x + dx, y: world.y + dy },
              finished.camera.zoom,
            ),
          );
        }
        setDraftPin(null);
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // enough to scroll, and a map that swallowed every wheel event would snag
      // the operator on the way past it. `ctrlKey` is both "Ctrl held" and how
      // every browser reports a trackpad pinch, so the one branch serves both.
      if (!e.ctrlKey && !e.metaKey) {
        raiseHint(
          isMacRef.current
            ? tRef.current("mapZoomHintMac")
            : tRef.current("mapZoomHint"),
        );
        return;
      }
  ...
  return (
    <Box flexDirection="column" gap={10}>
      ...
      >
        ...
        {/* The operator's own position, drawn by the shared marker: the same
            teardrop a customer gets on the public map. It carries no
            `PIN_CLASS`, so it is not draggable, and takes no pointer at all - a
            click that lands on it still drops the real pin underneath. */}
        {userOffset && (
          <OsmUserMarker
            left={userOffset.left}
            top={userOffset.top}
            label={t("mapYourLocation")}
          />
        )}

        {/* The coordinate itself, in the shared marker's geometry: a square with
            three round corners, rotated 45° about its **centre** so the sharp
            one points straight down, and offset by 120.7% of its own height so
            that corner - not the box - rests on the coordinate. `lib/map-capture`
            redraws exactly this by hand for the screenshot; keep the two in
            step. */}
        {pinOffset && (
          <div
        ...
        {/* The basemap's own credit, not a message key: it changes with the tile
            provider the tenant is set to, and a string in `messages/` cannot
            follow a setting an operator edits at runtime. */}
        <OsmAttribution
