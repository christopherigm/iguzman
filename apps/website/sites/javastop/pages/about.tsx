...
/**
 * "/about" for JavaStop - the long version of the story the landing's Intro only
 * teases: the family, the corner of Main Street they have kept since 1992, and
 * what the shop stands for (Mission/Vision). Every word and the photo are the
 * tenant's own `System` fields, so the family edits this page in the CMS and no
 * copy is frozen in the site folder. Served through the `[locale]/[...sitePath]`
 * catch-all from the site's `pages` map, and linked from the footer for free
 * (`components/footer.tsx` adds the link for any site declaring "/about").
 *
  ...
  return (
    <>
      ...
      >
        ...
        {/* Where the story leads: the menu (only once there is one to show), the
            wall the café gives to a local artist each month, and the shared
            /contact page, which already carries the address, the map, the hours
            and the form. */}
        <Box display="flex" gap="16px" flexWrap="wrap" alignItems="center">
          ...
          <Button text={t("about.artistsCta")} href="/artists" size="lg" />
