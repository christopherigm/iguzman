 ...
 * small requests rather than one per row. They go in parallel and the list state
 * is only updated once every write lands, so a failure leaves the table showing
 * the order the API still holds.
 */
export function useReorder(
  ...
  return useCallback(
    async (ordered: Items) => {
      ...
      const moved = ordered.filter((row, index) => Number(row.sort_order ?? 0) !== index);
      if (moved.length === 0) return;

      try {
        await Promise.all(
          moved.map((row) => update(row.id as number, { sort_order: ordered.indexOf(row) })),
        );
