...
export async function SightingsSection({ sightings, locale, title, subtitle }: Props) {
  ...
  const slides = sightings.map((sighting) => toSightingSlide(sighting, locale, format));
...
function toSightingSlide(
  ...
  return {
    ...
    // the locale lives.
    href: `/${locale}/sightings/${sighting.slug}`,
    speciesName,
