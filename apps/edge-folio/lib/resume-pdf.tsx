...
export function NaftaLetterDocument({
  companyName,
  letterText,
}: NaftaLetterPDFProps) {
    ...
    <Document>
      <Page size="LETTER" style={naftaStyles.page}>
        <View style={naftaStyles.companyHeader}>
          ...
          <Text style={naftaStyles.companyMeta}>
            TN NAFTA Visa Support Letter
          </Text>
        </View>
        ...
        {lines.map((line, i) => {
          ...
          if (trimmed.startsWith("•")) {
            return (
              <View key={i} style={naftaStyles.bullet}>
                ...
                <Text style={naftaStyles.bulletText}>
          ...
          if (isHeading(trimmed)) {
            return (
              <Text key={i} style={naftaStyles.sectionHeading}>
                {trimmed}
              </Text>
            );
          }
          return (
            <Text
              key={i}
              style={inHeader ? naftaStyles.headerLine : naftaStyles.paragraph}
            >
              {trimmed}
            </Text>
          );
...
export function ResumeDocument({
  ...
  for (const b of tailoredBullets) {
    if (b.work_experience_id) {
      if (!bulletsByWE.has(b.work_experience_id))
        bulletsByWE.set(b.work_experience_id, []);
            ...
            {(projects ?? []).map((proj) => (
              <View key={proj.id} style={{ marginBottom: 8 }}>
                <View style={styles.entryHeader}>
                  ...
                  {proj.url ? (
                    <Text style={styles.entryMeta}>{proj.url}</Text>
                  ) : null}
                </View>
