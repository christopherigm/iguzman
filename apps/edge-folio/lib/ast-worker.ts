...
const SKIP_DIRS = new Set([
  ...
  '.cache', 'coverage', '.turbo', 'target', 'vendor', '.idea', '.vscode',
  'out', '.output', '.tox', 'venv', '.venv', 'env',
]);
...
const FRAMEWORK_MAP: Record<string, string> = {
  ...
  angular: '@angular/core', svelte: 'Svelte', solid: 'Solid',
  express: 'Express', fastify: 'Fastify', koa: 'Koa', hapi: 'Hapi',
  django: 'Django', fastapi: 'FastAPI', flask: 'Flask', tornado: 'Tornado',
  spring: 'Spring', 'spring-boot': 'Spring Boot',
  nestjs: 'NestJS', '@nestjs/core': 'NestJS',
  laravel: 'Laravel', rails: 'Rails', phoenix: 'Phoenix',
  gin: 'Gin', fiber: 'Fiber', echo: 'Echo',
  graphql: 'GraphQL', prisma: 'Prisma', sequelize: 'Sequelize',
  mongoose: 'Mongoose', sqlalchemy: 'SQLAlchemy',
  celery: 'Celery', redis: 'Redis', kafka: 'Kafka',
  tensorflow: 'TensorFlow', pytorch: 'PyTorch', numpy: 'NumPy',
  pandas: 'Pandas', scikit: 'scikit-learn',
};

const CLOUD_HINTS: Record<string, string> = {
  aws: 'AWS', amazonaws: 'AWS', 'google-cloud': 'GCP', '@google-cloud': 'GCP',
  azure: 'Azure', '@azure': 'Azure', cloudflare: 'Cloudflare',
  vercel: 'Vercel', heroku: 'Heroku', digitalocean: 'DigitalOcean',
};

// ── Regex fallbacks (used when grammar WASM is unavailable) ───────────────────

const JS_IMPORT_RE = /(?:import\s+(?:[^'"]*?\bfrom\s+)?|require\s*\(\s*)['"]([^'"./][^'"]*)['"]/g;
const PY_IMPORT_RE = /^(?:from\s+([\w.]+)\s+import|import\s+([\w.]+(?:\s*,\s*[\w.]+)*))/gm;
...
function extractImportsAst(source: string, parser: TsParser, isPython: boolean): string[] {
  const results: string[] = [];
  try {
...
function extractImports(source: string, ext: string): string[] {
  const isJs = ['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.vue', '.svelte'].includes(ext);
  const isPy = ext === '.py';
...
function parsePackageJson(text: string): { runtime: string[]; dev: string[] } {
  try {
    ...
    const runtime = Object.keys((pkg.dependencies as Record<string, unknown>) ?? {});
...
function parseCargoToml(text: string): string[] {
  ...
  for (const line of text.split('\n')) {
    ...
    if (/^\[dependencies\]/.test(trimmed) || /^\[dev-dependencies\]/.test(trimmed)) {
      inDeps = true;
...
async function walk(
  ...
  for await (const [name, entry] of handle.entries()) {
    ...
    if (entry.kind === 'directory') {
      ...
      await walk(entry as FileSystemDirectoryHandle, rel, depth + 1, result, aborted);
      continue;
    }
    ...
    if (lower === 'dockerfile' || lower.startsWith('dockerfile.')) result.infra.hasDocker = true;
    if (lower === 'docker-compose.yml' || lower === 'docker-compose.yaml') result.infra.hasDocker = true;
    ...
    // K8s manifests
    if ((ext === '.yml' || ext === '.yaml') && !lower.includes('docker-compose')) {
      text = await tryRead();
...
async function runExtraction(handle: FileSystemDirectoryHandle): Promise<void> {
  ...
  const result: WalkResult = {
    ...
    importedModules: [],
    languages: new Set(),
    infra: { hasDocker: false, hasKubernetes: false, ciSystems: [], cloudHints: [] },
    kicadFiles: [],
    ciSystems: new Set(),
    totalFiles: 0,
    codeFiles: 0,
  ...
  // Deduplicate
  const uniqRuntime = [...new Set(result.runtimeDeps.map((d) => d.toLowerCase()))];
  ...
  const BUILTINS = new Set([
    ...
    'itertools', 'functools', 'pathlib', 'typing', 'abc', 'copy', 'enum', 'uuid',
    'fs', 'path', 'http', 'https', 'url', 'util', 'events', 'stream',
    'child_process', 'crypto', 'buffer', 'process', 'assert',
  ]);
  const uniqImports = [...new Set(
