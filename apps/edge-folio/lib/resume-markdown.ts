...
export function buildResumeMarkdown({
  ...
  if (workExperiences && workExperiences.length > 0) {
    ...
    for (const exp of workExperiences) {
      ...
      lines.push(
        `*${formatDateRange(exp.start_date, exp.end_date, exp.is_current)}${exp.location ? `  ·  ${exp.location}` : ""}*`,
      );
  ...
  if (projects && projects.length > 0) {
    ...
    for (const proj of projects) {
      ...
      if (proj.tech_stack && proj.tech_stack.length > 0) {
        lines.push(
          `**Tech:** ${proj.tech_stack.map((ts) => ts.name).join(", ")}`,
        );
      }
      if (proj.description) {
        lines.push("");
  ...
  if (educations && educations.length > 0) {
    ...
    for (const edu of educations) {
      ...
      const degree = edu.field_of_study
        ? `${degreeLabel} in ${edu.field_of_study}`
        : degreeLabel;
      lines.push(`### ${degree}`);
