...
function formatSalary(posting: JobPosting): string | null {
  const min = posting.salary_min ? Math.round(Number(posting.salary_min)) : null;
  const max = posting.salary_max ? Math.round(Number(posting.salary_max)) : null;
    ...
    <Card gap={8}>
      <Box display="flex" alignItems="flex-start" gap={10}>
        <Box
          ...
          styles={{ width: 48, height: 48, flexShrink: 0, borderRadius: 8, background: 'var(--surface-2)' }}
        >
        ...
        <Box display="flex" flexDirection="column" gap={2} flex={1} styles={{ minWidth: 0 }}>
          <Box display="flex" alignItems="center" gap={6} flexWrap="wrap">
            {posting.is_private && (
              <Badge variant="subtle" color="#8b5cf6" style={{ textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                {t('privateBadge')}
              </Badge>
            )}
            {posting.score > 0 && (
              <Badge variant="subtle" color="#06b6d4" style={{ textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                {t('matchBadge')}
              </Badge>
            )}
          </Box>
      ...
      <Box display="flex" gap={6} flexWrap="wrap">
        {posting.location && (
          <Badge variant="subtle" color="#6b7280">{posting.location}</Badge>
        )}
        {(posting.work_type ?? []).map((wt) => (
          <Badge key={wt} variant="subtle" color="#0ea5e9">{t(`workTypes.${wt}`)}</Badge>
        ))}
        {salary && <Badge variant="subtle" color="#22c55e">{salary}</Badge>}
      </Box>
      ...
      <Box display="flex" alignItems="center" justifyContent="space-between" marginTop={4} gap={8}>
        <Typography variant="caption" color="var(--muted-foreground, #6b7280)">
          {date}
        </Typography>
        <Box display="flex" gap={6} alignItems="center">
          ...
          {savedAppId != null ? (
            <Link href={`/${locale}/applications/${savedAppId}`} prefetch className="jobs__saved-link">
              {t('tailor')}
            </Link>
          ) : (
            <Button
...
export function JobsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getJobFeed({ country, work_type: workType, q, page, per: PER_PAGE });
  ...
  const filterChips = useMemo(
    () => (
      <Box display="flex" gap={12} flexWrap="wrap" alignItems="flex-end" marginBottom={20}>
        ...
        <Box styles={{ minWidth: 140 }}>
          <Select
            ...
            options={[
              ...
              ...COUNTRIES.map((c) => ({ value: c, label: t(`countries.${c}`) })),
            ]}
            width="100%"
          />
        </Box>
        <Box styles={{ minWidth: 140 }}>
          <Select
            ...
            options={[
              ...
              ...WORK_TYPES.map((w) => ({ value: w, label: t(`workTypes.${w}`) })),
            ]}
            width="100%"
          />
        </Box>
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <Box display="flex" flexDirection="column" alignItems="center" gap={16} paddingY={60}>
        ...
        <>
          ...
          {totalPages > 1 && (
            <Box display="flex" alignItems="center" justifyContent="center" gap={12} marginTop={24}>
              <Button
                text={t('prev')}
                type="button"
                size="md"
