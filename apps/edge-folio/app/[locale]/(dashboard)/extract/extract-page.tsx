      ...
      <Box className="extract__skeleton-card">
        <SkeletonRow label={t("skeletonFiles")}>
          <Typography variant="body">
            {t("skeletonFilesValue", {
              code: skeleton.fileStats.codeFiles,
              total: skeleton.fileStats.totalFiles,
            })}
          </Typography>
        </SkeletonRow>
        ...
        <SkeletonRow label={t("skeletonLanguages")}>
          {skeleton.languages.length > 0 ? (
            <Box display="flex" flexWrap="wrap" gap={4}>
              {skeleton.languages.map((l) => (
                <Badge key={l} variant="subtle" size="md">
                  {l}
                </Badge>
              ))}
            </Box>
        ...
        <SkeletonRow label={t("skeletonFrameworks")}>
          {skeleton.frameworks.length > 0 ? (
            <Box display="flex" flexWrap="wrap" gap={4}>
              {skeleton.frameworks.map((f) => (
                <Badge key={f} variant="outlined" size="md">
                  {f}
                </Badge>
              ))}
            </Box>
        ...
        {(skeleton.infra.hasDocker ||
          ...
          <SkeletonRow label={t("skeletonInfra")}>
            <Box display="flex" flexWrap="wrap" gap={4}>
              ...
              {skeleton.infra.ciSystems.map((ci) => (
                <Badge key={ci} variant="subtle" size="md">
                  {ci}
                </Badge>
              ))}
              {skeleton.infra.cloudHints.map((c) => (
                <Badge key={c} variant="subtle" size="md">
                  {c}
                </Badge>
              ))}
              {skeleton.kicadFiles.map((f) => (
                <Badge key={f} variant="outlined" size="md">
                  KiCad
                </Badge>
              ))}
        ...
        {skeleton.importedModules.length > 0 && (
          <SkeletonRow label={t("skeletonModules")}>
            ...
            >
              ...
              {skeleton.importedModules.length > 30 &&
                ` +${skeleton.importedModules.length - 30} more`}
            </Typography>
          </SkeletonRow>
        )}
      </Box>
...
function DirCard({
  ...
  dir: {
    ...
    topLevelEntries: { name: string; kind: "file" | "directory" }[];
  };
  onClear: () => void;
  t: ReturnType<typeof useTranslations<"ExtractPage">>;
}) {
    ...
    <Box className="extract__dir-card">
      ...
      {dir.topLevelEntries.length > 0 && (
        <Box display="flex" flexDirection="column" gap={8} marginTop={4}>
          ...
          <Box
            className="extract__dir-entries"
            role="list"
            aria-label={t("pickerContentsAriaLabel")}
          >
            {dir.topLevelEntries.map((entry) => (
              <Box
                key={entry.name}
                className="extract__dir-entry"
                role="listitem"
              >
                <Box aria-hidden={true} className="extract__dir-entry-icon">
                  {entry.kind === "directory" ? (
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none">
                      <path
                        d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2V17a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"
                        fill="currentColor"
                      />
                    </svg>
                  ) : (
    ...
    >
      <Box display="flex" alignItems="flex-start" gap={12}>
        <Button
          ...
          className={[
            "extract__check-btn",
            draft.included ? "extract__check-btn--checked" : "",
          ]
            .filter(Boolean)
            .join(" ")}
          ...
          aria-label={
            draft.included
              ? t("synthExclude", { n: index + 1 })
              : t("synthInclude", { n: index + 1 })
          }
        >
          {draft.included && (
            <svg
        ...
        >
          ...
          <Box display="flex" alignItems="center" gap={8} flexWrap="wrap">
            <Select
              ...
              options={CATEGORY_CHOICES.map((cat) => ({
                value: cat,
...
export function ExtractPage() {
  ...
  const handleDraftChange = useCallback(
    (id: string, patch: Partial<EditableDraft>) => {
      setDrafts((prev) =>
                    ...
                    >
                      <Button
                        text={
                          synthStatus === "saving"
                            ? t("synthSaving")
                            : t("synthSave", { count: includedCount })
                        }
                        type="button"
                        size="md"
                        kind="success"
