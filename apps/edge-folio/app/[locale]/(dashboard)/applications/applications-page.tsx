    ...
    async (urlOverride?: string) => {
      ...
      try {
        ...
        if (data.salary_currency)
          setSalaryCurrency(data.salary_currency as SalaryCurrency);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (err instanceof ApplicationError && err.status === 400) {
        ...
        if (
          Array.isArray(jobUrlErrors) &&
          (jobUrlErrors as string[]).includes("duplicate")
        ) {
...
function formatSalary(app: JobApplication): string | null {
  const min =
    app.salary_min != null ? Math.round(Number(app.salary_min)) : null;
  const max =
    app.salary_max != null ? Math.round(Number(app.salary_max)) : null;
