...
const SUMMARY_PARAGRAPH_COUNT_STEPS = [1, 2, 3].map((n) => ({ value: n, label: String(n) }));
...
const PROFICIENCY_OPTIONS: LanguageProficiency[] = ['native', 'fluent', 'professional', 'basic'];
...
export function ProfilePage() {
  ...
  const proficiencyOptions = PROFICIENCY_OPTIONS.map((p) => ({ value: p, label: t(`proficiencies.${p}`) }));
  ...
  const [newLangProficiency, setNewLangProficiency] = useState<LanguageProficiency>('professional');
  ...
  const [summaryShowEnhanceOptions, setSummaryShowEnhanceOptions] = useState(false);
  const [summaryEnhanceParagraphs, setSummaryEnhanceParagraphs] = useState(1);
  const [summaryEnhanceParagraphLength, setSummaryEnhanceParagraphLength] = useState('sm');
  ...
  const handleAddLanguage = useCallback(async () => {
    ...
    setAddingLang(true);
    try {
      const lang = await createLanguage({ name, proficiency: newLangProficiency, order: languages.length });
  ...
  const handleSaveLang = useCallback(async (id: number) => {
    ...
    try {
      ...
      setLanguages((prev) =>
        prev.map((l) => (l.id === id ? { ...l, name, proficiency: editLangProficiency } : l)),
      );
  ...
  const handleDeleteLang = useCallback(async (id: number) => {
    setLangError(null);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Section title={t('professionalSection')} subtitle={t('professionalSubtitle')}>
          ...
          <Box display="flex" flexDirection="column" gap={8}>
            <Box className="profile__field-label-row">
              ...
              <Box display="flex" alignItems="center" gap={6}>
                ...
                <Button
                  ...
                  iconSize="16px"
                  iconColor={summaryEnhancePreview ? 'var(--primary, #06b6d4)' : 'var(--foreground, #171717)'}
            ...
            {summaryEnhancePreview && (
              <Box className="profile__enhance-preview" flexDirection="column" gap={10}>
                <Typography variant="body-sm">{summaryEnhancePreview}</Typography>
                <Box display="flex" gap={8} alignItems="center" marginTop={12}>
                  {summaryIsGenerating ? (
                    <Button text={t('summaryEnhanceStop')} type="button" size="md" onClick={handleSummaryDiscardEnhance} />
                  ) : (
                    <>
                      <Button text={t('summaryEnhanceDiscard')} type="button" size="md" onClick={handleSummaryDiscardEnhance} />
                      <Button text={t('summaryEnhanceAccept')} type="button" size="md" kind="success" onClick={handleSummaryAcceptEnhance} />
                    </>
                  )}
                </Box>
              </Box>
        ...
        {/* ── Languages ── */}
        <Section title={t('languagesSection')} subtitle={t('languagesSubtitle')}>
          <Box display="flex" gap={8} flexDirection='column'>
            <TextInput
              label={t('langNameLabel')}
              type="text"
              value={newLangName}
              onChange={(v) => { setNewLangName(v); setLangError(null); }}
            ...
            <Box display='flex' alignItems='center' gap={12} marginBottom={20} >
              <Select
                ...
                onChange={(v) => { setNewLangProficiency(v as LanguageProficiency); }}
                ...
                editingLangId === lang.id ? (
                  ...
                  >
                    ...
                    <Select
                      ...
                      onChange={(v) => setEditLangProficiency(v as LanguageProficiency)}
                      options={proficiencyOptions}
                    />
                    <Button
                      text={savingLangId === lang.id ? t('langSaving') : t('langSave')}
                      type="button"
                      size="lg"
                      kind="success"
