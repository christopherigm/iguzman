package com.iguzman.mobforge.client;

import com.iguzman.mobforge.MobForge;
import com.iguzman.mobforge.entity.CubeEntity;
import net.minecraft.resources.ResourceLocation;
import software.bernie.geckolib.model.GeoModel;

/**
 * GeoModel binding the {@link CubeEntity} to its exported GeckoLib assets.
 *
 * <p>All three resource paths reuse the "cube" name verbatim (PRD R6). In
 * 1.20.2 the two-arg {@code new ResourceLocation(namespace, path)} constructor
 * is still current — the {@code fromNamespaceAndPath} rename lands in 1.20.5+.
 */
public class CubeModel extends GeoModel<CubeEntity> {

    private static final ResourceLocation MODEL =
            new ResourceLocation(MobForge.MOD_ID, "geo/cube.geo.json");
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(MobForge.MOD_ID, "textures/entity/cube.png");
    private static final ResourceLocation ANIMATION =
            new ResourceLocation(MobForge.MOD_ID, "animations/cube.animation.json");

    @Override
    public ResourceLocation getModelResource(CubeEntity animatable) {
        return MODEL;
    }

    @Override
    public ResourceLocation getTextureResource(CubeEntity animatable) {
        return TEXTURE;
    }

    @Override
    public ResourceLocation getAnimationResource(CubeEntity animatable) {
        return ANIMATION;
    }
}
