...
| Clickable button | `Button` | `text?` (or `children`), `href?`, `onClick?`, `type?`, `size?: 'sm'\|'md'\|'lg'` (default `'sm'`; sm=6px pad/13px, md=10px 18px/14px, lg=12px 24px/15px+r8), `onHover?`, `unstyled?` (strips default styles/wave — use for icon or toggle buttons with custom CSS), `disabled?`, `title?`, `aria-label?`, `aria-pressed?`, `aria-expanded?`, `aria-controls?` + UIComponentProps |
...
**Before writing any CSS, exhaust all props available on the `@repo/ui` component you are using.**
...
**Rule:** if a style value is static and does not depend on a CSS pseudo-class or media query, it belongs on the component prop — not in a CSS file. Dynamic values driven by React state should also go on the prop using a ternary.
