...
| Confirm dialog                   | `ConfirmationModal` | `title`, `text`, `okCallback`, `cancelCallback?`, `children?` (rendered between text and actions), `panelMaxWidth?` (CSS string, default `420px`), `position?: ModalPosition` (9-grid placement — `top-left` \| `top-center` \| `top-right` \| `center-left` \| `center` \| `center-right` \| `bottom-left` \| `bottom-center` \| `bottom-right`, default `'center'`), `backgroundBlur?` (CSS `backdrop-filter` value, default `'blur(2px)'`), `okDisabled?` |
