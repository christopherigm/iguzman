...
> **`Box` auto-applies `display: "flex"`.** Setting any flex *container* prop on a `Box` — `flexDirection`, `justifyContent`, `alignItems`, `flexWrap`, or `gap` — automatically sets `display: "flex"`, so you do **not** need to pass `display="flex"` alongside them. Pass `display` explicitly only to opt out (`display="grid"`, `display="block"`, etc.); an explicit `display` always wins. This inference is `Box`-only: flex *child* props (`flex`, `flexGrow`, `alignSelf`, `order`) never trigger it, and other components (`Grid`, `Card`) manage their own `display`.
