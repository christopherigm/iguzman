...
| Icon-only ghost button           | `IconButton`        | `icon` (required, SVG path), `aria-label` (required), `href?`, `target?` (applied to `next/Link` when `href` set; `'_blank'` auto-adds `rel="noopener noreferrer"`), `onClick?`, `type?`, `size?: 'sm'\|'md'\|'lg'` (default `'md'`; square box 28/36/44 px, auto icon 16/18/20 px), `kind?: 'default'\|'error'\|'success'\|'warning'` (default `'default'` - drives icon color + hover tint), `iconSize?`, `iconColor?` (defaults to color for `kind`), `disabled?`, `title?`, `isLoading?` (shows Spinner), `aria-pressed?`, `aria-expanded?`, `aria-controls?` + UIComponentProps. Transparent by default with a ghost hover tint; renders a native `button` or a `next/Link` (when `href` set and no `onClick`). |
| Text/underline link              | `LinkButton`        | `label` (required), `href?`, `onClick?`, `aria-label?`, `aria-current?`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Text field / textarea            | `TextInput`         | `label?`, `value?`, `onChange(v: string)?`, `type?`, `multirow?`, `rows?`, `placeholder?`, `onKeyDown?`, `min?`, `max?`, `step?`, `aria-label?` (used when `label` is omitted), `ref?` + UIComponentProps. Types `date`/`time`/`datetime-local`/`week`/`month`/`color` always keep the label floated (browser-native UI occupies the field area). All `NativeInputProps` not listed explicitly are also accepted.                                                                                                                                                                                                                                                          |
