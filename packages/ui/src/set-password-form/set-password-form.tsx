...
const SetPasswordForm = ({ URLBase, token, callback }: Props) => {
  ...
  return (
    ...
    >
      <Grid container columnSpacing={2} rowSpacing={2} maxWidth={600}>
            ...
            <Stack sx={{ width: '100%' }} spacing={2}>
              <Link href="/reset-password">
                <Alert severity="success">
                  ...
                  iniciar sesion dando click aqui.
                </Alert>
              </Link>
            </Stack>
          </Grid>
        ...
        {state.error.length &&
        state.error[0] &&
        (state.error[0].status === 500 || state.error[0].status === 404) ? (
          <Grid item xs={12} marginTop={2}>
            <Stack sx={{ width: '100%' }} spacing={2}>
              <Alert severity="error">
                Error: el codigo para crear la nueva contraseña es incorrecto o
                ha expirado, por favor contacta al administrador de la
                plataforma.
              </Alert>
              <Link href="/reset-password">
                <Alert severity="success">
                  Restablece tu contraseña de nuevo dando click aqui para
                  obtener un nuevo codigo.
                </Alert>
              </Link>
            </Stack>
          </Grid>
