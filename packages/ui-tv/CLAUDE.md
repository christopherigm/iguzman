...
| `@repo/ui-tv/tv-button` → `TvButton` | Focusable button (`onPress` fires on remote Enter); `kind` (`primary`/`success`/`error`/`warning`) tints it **only while focused** (so the focus ring reads against it), mirroring `@repo/ui`'s Button. `selected` is a persistent toggled state (e.g. a filter chip) that keeps the accent fill whether or not the button holds focus. `focusKey` gives the node a stable key so another component can restore focus via `setFocus(focusKey)`; `scrollOnFocus` scrolls the button into view on focus (use inside a scrollable list/modal); `onFocusChange` fires when the button gains focus (focus-driven side effects); `onArrowPress(direction)` intercepts an arrow while focused - return `false` to block default navigation. `icon` (imported SVG URL) renders a glyph via a `currentColor` CSS mask so it inherits the button's focus/kind tint, positioned by `iconPosition` (`'start'` default / `'end'`). |
| `@repo/ui-tv/tv-icon-button` → `TvIconButton` | Icon-only square button - the 10-foot counterpart of `@repo/ui`'s `IconButton`. Requires `icon` (imported SVG URL, masked to `currentColor` like `TvButton`'s) and `ariaLabel` (icon-only, so there is no text to announce; also used as `title`). `size` (`sm`/`md`/`lg` - `md` matches a `TvButton`'s height, so they sit flush in a row); `kind`, `selected`, `disabled`, `focusKey`, `scrollOnFocus`, `onFocusChange`, `onArrowPress` all behave as on `TvButton`. Unlike the web `IconButton` (a transparent ghost that reveals itself on hover), it reads as a solid surface-2 tile at rest - a TV has no hover, so the button must be discoverable without one. |
